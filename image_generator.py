...
使用 Gemini 2.0 Flash（免費方案）生成背景圖，再用 Pillow 合成文字
"""
    ...
    for path in FONT_PATHS:
        if os.path.exists(path):
            try:
                ...
                continue
    print("⚠️  找不到中文字體，使用預設字體")
    return ImageFont.load_default()


# ─── Gemini 2.0 Flash 圖片生成 ───────────────────────────────────────────────

def generate_background(prompt: str, api_key: str) -> Image.Image:
    """
    使用 google-genai SDK + gemini-2.0-flash-exp
    免費額度：每分鐘 10 次、每天 1500 次
    """
    try:
        ...
        raise ValueError("回應中未包含圖片資料")

    except Exception as e:
        print(f"⚠️  Gemini 失敗，使用備用漸層背景: {e}")
